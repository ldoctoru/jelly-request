    ...
    for attempt in range(1, max_retries + 1):
        try:
            with requests.Session() as session:
                ...
                res = session.get(f"{JELLYSEERR_URL}/api/v1/search", params={"query": encoded}, headers=headers, timeout=(5, 15))
...
def main():
    logger.info(f"Jelly Request started (4K: {IS_4K_REQUEST}, Auto-Approve: {AUTO_APPROVE})")
    while True:
        try:
